...
def optimize_model(X_train, y_train, n_trials=50):
    ...
    print(f"\nando hiperparámetros ({n_trials} trials)...")
...
def plot_results(df, model, X_test, y_test, y_pred, metrics):
    ...
    avg_days = df.groupby('ESCUELA_PROFESIONAL')[
        'DIAS_DESDE_EGRESO'].mean().sort_values()
...
def main():
    ...
    print(f"\n ¡Análisis completado!")
    print(
        f"🎯 El modelo puede predecir los días desde egreso con un R² de {metrics['test_r2']:.3f}")
